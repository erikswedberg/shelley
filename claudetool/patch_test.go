...
func TestPatchTool_FlexibleInputParsing(t *testing.T) {
	...
	// Test singular stringified patch format (PatchInputOneSingularString):
	// {"path": ..., "patch": "<json string>"} -- observed in the wild when the
	// model double-encodes the patch object as a JSON string. Previously
	// failed with: cannot unmarshal string into Go struct field
	// PatchInputOneSingular.patch of type claudetool.PatchRequest
	singularStr := `{"path": "` + testFile + `", "patch": "{\"operation\": \"replace\", \"oldText\": \"Modified\", \"newText\": \"Singular\"}"}`
	result = patch.Run(ctx, json.RawMessage(singularStr))
	if result.Error != nil {
		t.Fatalf("singular string patch format failed: %v", result.Error)
	}

	content, _ = os.ReadFile(testFile)
	if !strings.Contains(string(content), "Singular") {
		t.Error("singular string patch format did not work")
	}
}
